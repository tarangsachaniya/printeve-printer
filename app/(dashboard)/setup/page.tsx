  ...
  const handleNext = useCallback(async () => {
    ...
    try {
      if (step === 0) {
        ...
        if (!canvasRef.current) throw new Error('Signature canvas not ready')
        const signature_url = await uploadSignatureToCloudinary(canvasRef.current, accountHolder)
            ...
            <div className="space-y-5">
              ...
              {agreed && (
                <div className="space-y-3 pt-1">
                  ...
                  <SignatureCanvas canvasRef={canvasRef} onSigned={setSigned} />
                </div>
              )}
            </div>
          )}
